...
def demo_agent(env, model, seed=None, render=False, n_episodes=1):
    for ep in range(n_episodes):
        total_reward = 0
        steps = 0
        s, info = env.reset(seed=seed)
        while True:
            a = model.predict(s)[0].item()
            ...
            steps += 1
            if steps % 20 == 0:
                print(f"Completed {steps} steps.. ", end="\r")
            if terminated or truncated:
                if "episode" in info:
                    print(f"End of Episode {ep+1}")
                    print(
                        f"Episode Length = {info['episode']['l'][0]:.2f}, Reward = {info['episode']['r'][0]:.2f}"
                    )
                break
        if render:
            env.close()
...
class LunarLanderCustom(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        ), "Please specify a reward function for the environment!"
    ...
    def step(self, action):
        ...
        if hasattr(self, "state"):
            ...
            prev_state = np.zeros_like(self.observation_space.sample())
        ...
        reward = self.get_reward(self, prev_state, action, self.state)
...
if __name__ == "__main__":
    env = LunarLanderCustom(render_mode="human", enable_wind=True, wind_power=5.0)
